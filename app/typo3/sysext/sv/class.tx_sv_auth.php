...
class tx_sv_auth extends tx_sv_authbase 	{
	...
	function getGroups($user, $knownGroups)	{
		...
		if($this->mode=='getGroupsFE')	{
			...
			if (count($groups))	{
				...
				$lockToDomain_SQL = ' AND (lockToDomain=\'\' OR lockToDomain IS NULL OR lockToDomain=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($this->authInfo['HTTP_HOST'], $this->db_groups['table']) . ')';
	...
	function getSubGroups($grList, $idList='', &$groups)	{
		...
		$lockToDomain_SQL = ' AND (lockToDomain=\'\' OR lockToDomain IS NULL OR lockToDomain=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($this->authInfo['HTTP_HOST'], 'fe_groups') .')';
		if (!$this->authInfo['showHiddenRecords'])	$hiddenP = 'AND hidden=0 ';
