...
class tx_cssstyledcontent_pi1 extends tslib_pibase {
	 ...
	 */
	 function render_textpic($content, $conf)	{
      ...
      // Max Width override through maxWInText2_enabled
    if ( $this->cObj->stdWrap($conf['maxWInText2_enabled'], $conf['maxWInText2_enabled.']) == "1" ){
      $maxW = $this->cObj->stdWrap($conf['maxWInText2'], $conf['maxWInText2.']);
    }
      //if imageorient = image above text (2), render image in full width and ignore maxWInText2_enabled switch
    if( $this->cObj->stdWrap($conf['textPos'], $conf['textPos.']) == "2" ){
      $maxW = intval($this->cObj->stdWrap($conf['maxW'], $conf['maxW.']));
    }
      // Max Width override through maxWInText2_enabled end
