<?php 
/*
 * @description detects mobile browsers based on their user agent string
 * code taken from http://detectmobilebrowsers.mobi/
 * @todo  test on real devices
 * @param none
 * @return  boolean
 */

function user_isMobileBrowser(){
  $mobile_browser   = false; // set mobile browser as false till we can prove otherwise
  $user_agent       = t3lib_div::getIndpEnv('HTTP_USER_AGENT'); // get the user agent value - this should be cleaned to ensure no nefarious input gets executed
  $accept           = t3lib_div::getIndpEnv('HTTP_ACCEPT'); // get the content accept value - this should be cleaned to ensure no nefarious input gets executed
  
  //consider devices as mobile or not
  $fennec=true;
  $iphone=true;
  $ipad=false;
  $android=true;
  ...
  switch(true){ // using a switch against the following statements which could return true is more efficient than the previous method of using if statements
    ...
    case (preg_match('/fennec/i',$user_agent)); // we find the word fennec in the user agent
      $mobile_browser = $fennec; // mobile browser is either true or false depending on the setting of ipad when calling the function
  ...
  /**
   * return value
   * true -> mobile browser detected
   * false -> no mobile browser detected
   */
